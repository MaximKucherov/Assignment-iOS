...
//        viewModel.searchResults.count
        viewModel.citySearchResults.count
    }
...
//        let weatherInfo = viewModel.searchResults[indexPath.row]
        let weatherInfo = viewModel.citySearchResults[indexPath.row]

//        cell.textLabel?.text = "\(weatherInfo.name ?? ""), \(weatherInfo.state ?? ""), \(weatherInfo.country ?? "")"
        cell.textLabel?.text = "\(weatherInfo.name ?? ""), \(weatherInfo.country ?? "")"
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let selectedWeatherInfo = viewModel.citySearchResults[indexPath.row]

        let detailVC = DetailViewController()
        detailVC.cityLocationMeteoSource = selectedWeatherInfo
...
//        }
        ...
        viewModel.fetchLocationCity(for: searchTerm) { result in
            switch result {
            case .success:
                DispatchQueue.main.async {
                    self.tableView.reloadData()
